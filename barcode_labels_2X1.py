...
# path to folder where the label pdf's are saved. The folder must be created prior to executing this code.
save_to_folder = 'labels_supply'
# prefix added to item_name for quick identification. item_name is added to the end of the prefix for the full pdf name.
label_name_prefix = 'barcode_supply'

# label size variable which is equivalent to 2" X 1" used in the next step creating canvas (pagesize=small_label).  
small_label = (50.0*mm, 25.4*mm)

# create a canvas object using canvas.Canvas(where to save the generated pdf, set the label size)
...
# now create the actual PDF, must save the page before starting another page. no settings are saved between pages.
c.showPage()
c.save()
